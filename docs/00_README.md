...
本テンプレートは、**コンテンツ主体サイト（コーポレートサイト・サービス LP・ブログ / お知らせ・ドキュメント）+ 軽量な管理画面（CMS）** の事業・プロダクト・開発・運用・ガバナンス仕様を、抜け漏れなく定義するための「正仕様書テンプレート」です。本リポジトリ（`walk-pet-kampo`）の土台である `site-template`（Astro SSR + Svelte islands + Cloudflare Workers/D1/R2）は §0-1 の「パターン A」専用テンプレートであり、マルチテナント SaaS・業務システム（パターン B）は対象外です（本プロジェクトが「パターン A」の範囲を超える経緯は GOV-01 D-006）。
...
| 利用区分 | 概要 | 推奨フロー |
| --- | --- | --- |
| **自社プロジェクト** | 自社が運営するコンテンツサイト/サービスサイトの新規立ち上げ | フルフロー（全 26 文書） |
...
├── 3-development/           ← 開発仕様（DEV-01〜11。DEV-01 が技術スタック決定書）
...
| DEV-11 | 3-development/11-implementation-phases.md | 実装フェーズ計画（機能実装の順序と依存。日程は持たない） | 3 | Tech Lead |
...
長期運用前提のサービス。全 26 文書をフェーズ 0〜5 の順で埋める。生成順序の詳細は §9（正本）を参照。DEV-11（実装フェーズ計画）のみ他と性格が異なり、仕様が揃い実装に着手する段階で書く。
...
flowchart TD
    ...
    APR -->|承認| PHASE[DEV-11 実装フェーズ計画<br/>順序と依存のみ・日程は持たない]
    PHASE --> IMPL[実装着手]
```

- **本プロジェクトでは AI 機能は不採用のため、`PRD-03 -.AI機能あり時.-> PRD-05` の破線分岐は通らない**（`Decided` — GOV-01 D-005、PRD-05 §1）。PRD-05 は「不採用」を記録した状態で残し、採用に転じた場合のみ本文を記入して分岐を有効化する
