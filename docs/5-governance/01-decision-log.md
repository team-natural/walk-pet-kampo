...
### D-036：電話番号確認は登録時ではなく初回予約時に行う

| 項目 | 内容 |
| --- | --- |
| 日付 | 2026-09-23 |
| カテゴリ | 設計 / 事業 |
| 決定内容 | F-01-02（電話番号確認）の実施タイミングを **登録時 → 初回予約時** に移す。`WalkerProfile` の `pending_verification → active` の条件は「メール確認 + 利用規約同意」とし、電話確認は `walker_profiles.phone_verified_at` として**予約作成時に別途検証**する（`requirePhoneVerified`）。確認は 1 回のみで、2 回目以降の予約では通過する。SMS 自体を廃止するのではなく、対象を「予約する利用者」に絞る |
| 背景 | **費用**: SMS は 1 通あたり十数円規模の従量課金で、1 予約あたりの運営取り分は ¥100（BIZ-03 §2-1: 参加費 ¥500 − 団体還元 ¥400）。登録者全員に送ると、予約しなかった利用者の送信費がそのまま損失になり、登録→予約の転換率が低いほど悪化する。予約時に寄せれば費用は売上の立つ利用者にのみ発生する。**安全**: 電話確認が守っているのは ①事故時（FG-12 の咬傷・脱走）に当日つながること ②制限・停止した利用者の再登録を抑止すること の 2 点で、どちらも「予約が成立した利用者」に対して必要になる性質のもの。メール確認での代替は検討したが、①②のいずれも代替できない（打ち間違えた番号でもメール確認は通り、メールアドレスは無制限に作れる）ため採らない。**残るリスク**: 予約しない利用者は電話未確認のまま `active` になる。`active` は「利用資格あり」であって「連絡先検証済み」ではない、という区別を DEV-09 §2-4-4 に明記した |
| 影響範囲 | PRD-03 F-01-02、DEV-02 §1-2・§3-1・§3-2、DEV-04 §2、DEV-09 §2-4-3・§2-4-4・§2-7-3、DEV-11 P4・P11、GOV-02 TBD-61。**スキーマ変更は不要**（`walker_profiles.phone_verified_at` は既存 — DEV-07 §5-3） |
| 決定者 | Tech Lead（費用面の起案は事業側） |
| 関連 TBD | TBD-61（SMS の送信手段。必要になるのは P11 に後退）。**TBD-17/18/20（保険・責任分担）で覆る可能性がある** — 保険側が本人確認の水準を条件にする場合、本決定を再検討する |
