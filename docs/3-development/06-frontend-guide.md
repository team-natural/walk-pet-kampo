...
- **保護団体ページ**は `apps/public/src/lib/components/organization/` にテーブル/フォーム/カード/
  ...
  コンポーネントを乱立させない。
- 確認モーダル（`confirm-dialog.svelte`）は `action`（モーダル内の空フォームが POST する）と
  `form`（ページ側のフォームの `id` を渡し、その入力ごと送信する）のどちらかを受け取る。
  **入力欄を伴う破壊的操作では `form` を使う** — ADM-23 の退会理由のように、ページ側のフォームと
  モーダル側のフォームが別物だと入力が送信されずに落ちる。
- **ページが描画するデータは view model 経由で受ける**（`Decided` — GOV-01 D-029）。各画面は
  `apps/*/src/lib/view-models/` の型 1 つに束縛した `const` を持ち、Service 実装前はそこに
  `lib/mocks/` の仮データを代入する。UI 実装でこの束縛を外し、テンプレートに値を直書きしては
  ならない — Service を繋ぐときに 1 行の差し替えで済まなくなる。金額・キャンセル規定のような
  定数も同様で、`lib/pricing.ts` から読む（DEV-06 §1-1）。
