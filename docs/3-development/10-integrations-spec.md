...
**送信手段が未定義（GOV-02 TBD-61）。** PRD-03 F-01-02「電話番号確認（SMS）」は MVP `○` だが、
DEV-01 §2 の機能別標準ライブラリに SMS プロバイダが無く、本書にも連携仕様が無い。DEV-09 §2-4-3
の `pending_verification → active` の条件に含まれるため、予約フローのクリティカルパス上にある。

採用する場合は DEV-01 §2 に追記（Workers から fetch で呼べること・`nodejs_compat` 不要である
ことを確認）した上で、§8-5 のフォーマットで本節に連携仕様を書き起こす。MVP から外す場合は
PRD-03 F-01-02 の MVP 判定と DEV-09 §2-4-3 の遷移条件を同時に変更する。

### 8-5. プロダクト固有連携の追記フォーマット

本項執筆時点で決済（§2）・メール（§3）・ストレージ（§4）・ジオコーディング（§9）以外のプロダクト固有の外部連携は想定されていない（§8-4 の SMS を除く）。今後追加が必要になった場合は、以下のフォーマット（連携先 / 認証方式 / 呼び出し方式 / Webhook / エラー処理）で本節に追記する。
